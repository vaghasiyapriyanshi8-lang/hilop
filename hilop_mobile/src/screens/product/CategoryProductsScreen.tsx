...
type Props = {
  navigation: NativeStackNavigationProp<HomeStackParamList, 'CategoryProducts'>;
  route: RouteProp<HomeStackParamList, 'CategoryProducts'>;
};
...
export default function CategoryProductsScreen({ navigation, route }: Props) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['categoryProducts', categoryId, sort, page],
    queryFn: () =>
      apiClient.get('/products', {
        params: { category: categoryId, sort, page, limit: 12 },
      }).then(r => r.data),
  });
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      {!isLoading && (
        <Text style={styles.resultCount}>
          {data?.total ?? products.length} watches in {categoryName}
        </Text>
      )}
      ...
      {isLoading ? (
        ...
        <View style={styles.centered}>
          ...
          <Text style={styles.emptyText}>
            No watches available in {categoryName} right now
          </Text>
        </View>
      ) : (
        <FlatList
